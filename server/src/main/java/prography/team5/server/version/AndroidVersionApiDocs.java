...
@Tag(name = "0-2. Android 버전", description = "Android 버전 관련 기능입니다.")
public interface AndroidVersionApiDocs {
    ...
    @Operation(
            summary = "[Android] 모든 버전 조회 API",
            description = "모든 버전을 조회할 수 있습니다."
    )
    @ApiResponse(responseCode = "200", description = "모든 버전 조회 성공입니다.")
    ResponseEntity<CommonApiResponse<List<VersionCheckResponse>>> findAllAndroidVersions();
